...
def search_privacy_law(query: str) -> str:
    """Search privacy law and data protection obligations.
    ...
    knowledge = [
        (
            ["gdpr", "eu", "privacy", "data", "breach"],
            "GDPR: fines up to 4% of global annual turnover or EUR 20M (whichever is higher). "
            "Data controllers must notify supervisory authority within 72 hours for qualifying breaches.",
        ),
        (
            ["ccpa", "cpra", "california", "consumer"],
            "CCPA/CPRA: statutory damages for certain data breaches ($100-$750 per consumer per incident), "
            "plus civil penalties by California AG/CPPA.",
        ),
        (
            ["consent", "personal data", "processing"],
            "Core principles: lawfulness, fairness, transparency, purpose limitation, data minimization, "
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, CCPA, data breach'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        ...
        "cross-border data transfers, incident response, and data breach notification obligations. "
